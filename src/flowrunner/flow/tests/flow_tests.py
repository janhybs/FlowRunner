...
from flowrunner.runner.execution.plugins.plugin_print_command import PluginPrintCommand
from flowrunner.runner.execution.plugins.plugin_progress import PluginProgress
from flowrunner.runner.execution.plugins.plugin_timer import PluginTimer
from flowrunner.runner.execution.utils.exec_util import check_output_secure, exec_single, prepare_all
from flowrunner.utils import merge_dict
from flowrunner.utils.io import browse, mkdir
from flowrunner.utils.pluck import pluck
...
class FlowTester(object):
    def __init__(self, **kwargs):
        ...
        # filter folders
        all_tests = lists.filter(all_tests, lambda x: os.path.isdir(io.join_path(self.tests_root, x)))
    ...
    def browse_test_config_files(self, test_name, test_option):
        ...
        test_option['problem'] = lists.filter(test_option['problem'], lambda x: bool(x))
        test_option['problem'] = lists.filter(test_option['problem'], lambda x: self.select_ini_rule.match(x))
    ...
    def setup_test_paths(self, test_name, test_option):
        ...
        test_option['problem_config'] = lists.prepend_path(test_option['problem'], self.tests_root, test_name)
